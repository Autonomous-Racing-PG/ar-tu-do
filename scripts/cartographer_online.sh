...
rosbag record -j -O $scriptdir/../ros_ws/src/navigation_stack/car_cartographer/files/map.bag scan imu tf __name:=rosbag_recording 2>&1 | sed -r "s/([[:cntrl:]]\[[0-9]{1,3}m)|(..;?)|//g" >> $logpath &
...
if [ $ROS_DISTRO == "kinetic" ] ; then
    ...
    rosservice call /write_assets "$scriptdir/../ros_ws/src/navigation_stack/car_cartographer/files/map" | sed -r "s/([[:cntrl:]]\[[0-9]{1,3}m)|(..;?)|//g" >> $logpath
    echo "Done." | tee -a $logpath
fi
if [ $ROS_DISTRO == "melodic" ] ; then
    echo -n "Write map stream... " | tee -a $logpath
    rosservice call /write_state "{filename: '$scriptdir/../ros_ws/src/navigation_stack/car_cartographer/files/map.bag.pbstream'}" | sed -r "s/([[:cntrl:]]\[[0-9]{1,3}m)|(..;?)|//g" >> $logpath
    echo "Done." | tee -a $logpath
fi
...
if [ $ROS_DISTRO == "melodic" ] ; then
    echo -n "Write map... " | tee -a $logpath
    echo "" >> $logpath
    roslaunch car_cartographer cartographer_make_map.launch bag_filenames:=$scriptdir/../ros_ws/src/navigation_stack/car_cartographer/files/map.bag pose_graph_filename:=$scriptdir/../ros_ws/src/navigation_stack/car_cartographer/files/map.bag.pbstream 2>&1 | sed -r "s/([[:cntrl:]]\[[0-9]{1,3}m)|(..;?)|//g" >> $logpath
    echo "Done." | tee -a $logpath
fi
