...
if [ $ROS_DISTRO == "kinetic" ] ; then
    ...
    echo "" >> $logpath
    roslaunch car_cartographer cartographer_offline_fast.launch bag_filenames:=$1 ros_version:=$ROS_DISTRO 2>&1 | sed -r "s/([[:cntrl:]]\[[0-9]{1,3}m)|(..;?)|//g" >> $logpath
    echo "Done." | tee -a $logpath
fi
...
if [ $ROS_DISTRO == "melodic" ] ; then
    ...
    echo -n "Create map... " | tee -a $logpath
    echo "" >> $logpath
    roslaunch car_cartographer cartographer_make_map.launch bag_filenames:=$1 pose_graph_filename:=$1.pbstream 2>&1 | sed -r "s/([[:cntrl:]]\[[0-9]{1,3}m)|(..;?)|//g" >> $logpath
    echo "Done." | tee -a $logpath
fi
