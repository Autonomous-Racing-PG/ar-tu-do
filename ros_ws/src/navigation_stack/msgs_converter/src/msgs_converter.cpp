...
MSGSConverter::MSGSConverter()
{
    this->m_command_velocity_subscriber =
        this->m_node_handle.subscribe<geometry_msgs::Twist>(car_config::CMD_VEL, 1,
                                                         &MSGSConverter::convertCallback, this);
...
void MSGSConverter::convertCallback(const geometry_msgs::Twist::ConstPtr& cmd_vel_message)
{
    ...
    double velocity_result = metric_velocity.norm(); //check if norm is zero, then the steering angle (angle_rad) will be infinity!
    ...
    control.angle = (servo_data*2 - car_config::MAX_SERVO_POSITION) / car_config::MAX_SERVO_POSITION; //convert from 0-1 to (-1)-1
