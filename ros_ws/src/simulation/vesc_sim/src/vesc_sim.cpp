...
VESCSimulator::VESCSimulator()
{
    ...
    m_optim_y_pos = new LeastSquares1(window_size,1/m_frequency);
    m_optim_cur_ang_vel = new LeastSquares1(window_size,1/m_frequency);
...
VESCSimulator::~VESCSimulator()
{
    ...
    delete m_optim_x_pos;
    delete m_optim_y_pos;
    delete m_optim_cur_ang_vel;
}
...
void VESCSimulator::timerCallback(const ros::TimerEvent& event)
{
    ...
    m_current_angular_velocity = m_optim_cur_ang_vel->optimize(m_current_speed * tan(m_current_steering_angle) / car_config::WHEELBASE); // rad/s
    ...
    // propagate odometry
    m_x_dot = m_optim_x_dot->optimize(m_current_speed * cos(m_yaw));
    m_y_dot = m_optim_y_dot->optimize(m_current_speed * sin(m_yaw));
